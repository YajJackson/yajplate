...
export const SignUpForm = () => {
    ...
    const disableButton = useMemo(
        () => !(data.email && data.password) || data.password !== confirmPassword,
    ...
    return (
        <div className='flex flex-col space-y-6 w-full'>
            <AnimateSharedLayout>
                ...
                <div>
                    <label htmlFor='about' className='block text-sm font-medium text-gray-700'>
                        {selectedInput == INPUT_SELECTED.PASSWORD_CONFIRM && (
                            <motion.div
                                layoutId='fun'
                                className='bg-primary h-8 w-1 fixed rounded-full'
                                initial={{ x: -20, y: 25 }}
                        ...
                        <input
                            ...
                            onFocus={() => setSelectedInput(INPUT_SELECTED.PASSWORD_CONFIRM)}
                        />
                    </label>
                </div>
                {!disableButton && (
                    <div className='flex flex-col items-center'>
                        <motion.button
                            onClick={() => !disableButton && registerUser()}
                            key='continue-button'
