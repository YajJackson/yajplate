...
export const SignUpForm = () => {
    ...
    const registerUser: SubmitHandler<Inputs> = async (data) => {
        setLoading(true)
        const credentials = { ...data, username: data.email, display_name: data.email }
    ...
    return (
        <form onSubmit={handleSubmit(registerUser)} className='flex flex-col space-y-6 w-full'>
            <AnimateSharedLayout>
                {/* Email */}
                <div>
                ...
                <div>
                    <label htmlFor='about' className='block text-sm font-medium text-gray-700'>
                        ...
                        <input
                            ...
                            className='mt-1 shadow-sm transition focus:ring focus:ring-gray-300 focus:border-black focus:outline-none block w-full sm:text-sm border border-gray-300 rounded-md p-2'
                            type='password'
                            onFocus={() => setSelectedInput(INPUT_OPTION.PASSWORD_CONFIRM)}
                        />
                    </label>
                </div>
                ...
                {isValid && (
                    <motion.button
                        type='submit'
                        className='text-white bg-primary md:opacity-95 md:hover:opacity-100 px-4 py-2 rounded-lg focus:outline-none float-left shadow-app w-full'
